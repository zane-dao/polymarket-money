...
| `tests/`、`execution/tests/` | WSL Linux Node 下 TypeScript contract 可编译；63 项 Python 与 40 项 Node 离线测试覆盖第一批黄金规则、raw contract、市场身份、manifest/replay、public adapter、存储和负向边界；干净 venv/npm 安装通过 | integration/shadow 仍为空；有限 smoke 不能证明长期协议连续性、回测、恢复或实盘执行正确 |
...
`batch/2-readonly-data` 完成只读数据链。该基线不把 raw replay、离线模型或有限 smoke
误报成持久订单账本、可信回测或实盘能力。
