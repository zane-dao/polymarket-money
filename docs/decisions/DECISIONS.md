...
## D-061 B0–B3 必须作为自动执行的竞技场门槛，而不是策略页说明卡

**时间：2026-07-23 01:45 Asia/Singapore**

B0–B3 继续作为冻结、只读、不可编辑的研究基线保留，但产品位置改为回测与策略竞技场：用户启动一次
J/K/L 回测，后端用同一数据集、分组、费用、延迟、资金和仓位假设自动追加 B0–B3，竞技场直接显示
候选是否超过最强基线。策略工作室不再展示无操作、无结果的基线说明卡。

本决定取代 D-060 中“在策略工作室默认可见”的具体展示安排，但保留其“不随意新增、隐藏、删除既有
资产；先证明用途和调用者”的治理原则。基线不得进入 Paper，也不得成为用户手工配置步骤。

## D-060 既有资产先分类再改动，禁止在隐藏、恢复和删除之间反复摆动

**时间：2026-07-23 00:26 Asia/Singapore**

任何既有页面、策略、研究基线或运行模块在调整前，必须先依据真实用途与调用者明确归入三类之一：
“保留并进入主流程”“保留为清楚可见的只读/归档资产”“确认无价值且无调用者后删除”。实施前后都要保持
这一分类稳定，并在交付说明理由；不得为了单次界面整理把同一资产先隐藏、再恢复或无依据删除，造成
用户重新理解和重复审阅。

B0–B3 已确定为长期保留的只读研究对照基线，用于判断复杂策略是否优于不交易、市场概率、GBM 和
逻辑回归。它们在策略工作室默认可见，但不混入可编辑策略、回测启动选择或 Paper 主流程。L V1 属于
历史门失败策略，继续单列只读审计；J、K、L V2 才进入各自允许的编辑或运行流程。只有用途、调用者或
研究结论发生了可验证变化，才允许重新分类，并同步更新本决定和界面说明。

## D-059 运行时代码改动必须重启候选进程并回读实际版本

**时间：2026-07-22 23:42 Asia/Singapore**

凡改动当前 Web 前端、Web 后端、策略 catalog 或 Paper 运行链，任务完成条件必须包含执行
`npm run release:candidate`，使 4273 切换到基于当前工作树构建的新不可变 release。仅完成构建、测试
或临时端口验收，不能表述为常驻前后端已经更新。

重启后必须从 4273 实际 API 回读准确 release ID、`staging-sim` 和
`LIVE_TRADING_ENABLED=false`，并核对至少一个本次变更特征。仅文档、注释或不进入运行产物的测试
改动可不重启。若可选的 `polymarket-vite-4174.service` 当时正在运行，候选刷新还必须重启该服务并
验证 4174 页面可用；未运行时不主动创建。4173 stable 仍只按独立晋升流程更新，不能被候选刷新命令
连带重启。

## D-058 Web 产品只保留一个自动 Paper Runner 主流程

**时间：2026-07-22 23:00 Asia/Singapore**

当前产品入口固定为本机 Web。用户流程只有选择策略版本、填写初始资金/最大仓位/最低净优势、启动、
查看结果、停止或紧急停止。当前及后续 BTC 五分钟市场发现、公开行情、标准化输入、策略目标仓位、
统一风控、模拟成交、账本和官方结算由一个后端 Runner 自动编排。slug、host、session、journal、
outbox、手工订单和手工结算只可作为内部诊断，不再是用户步骤。

`strategies/catalog.json` 是策略元数据、内置冻结版本、参数 Schema、允许模式与执行器键的唯一登记
源；新数据根直接获得可选的 Paper-ready 版本，不以手工创建版本作为启动前置步骤。Tauri 保留为
暂停资产，但不属于当前主链或验收对象。`LIVE_TRADING_ENABLED=false` 持续不变；启动公开采集仍需
用户明确操作，且不引入钱包、签名、私有频道或真实订单。

## D-057 测试候选发布必须重启并回读实际运行版本

**时间：2026-07-22 22:05 Asia/Singapore**

`npm run release:candidate` 不是单纯打包动作：它必须构建 candidate、只重启 4273 的
`polymarket-staging-4273.service`（服务尚不存在时仅启动它），再经固定本机 API 回读 `staging-sim`、
准确 release ID 与 `liveTradingEnabled=false`。任一项不符即失败，不能把工作树或 candidate 指针误当作
正在运行的后端。

该命令还固定验证 J/K 的空优势区间和 L V2 的零最大信号优势均返回带严重度的研究告警且 `valid=true`，
防止旧的硬拒绝文案随发布回归。4173 stable、网络采集与真实交易均不属于这个命令的范围。
