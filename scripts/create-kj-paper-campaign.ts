...
async function main(): Promise<void> {
  if (process.argv.includes("--help")) {
    process.stdout.write([
      "Usage: npm run paper:campaign-plan -- --campaign-id research-20260718 --first-full-market-start 2026-07-18T12:00:00.000Z --runs 3 --output /absolute/new-campaign.json",
      "       [--markets 3] [--gap-markets 0] [--settlement-grace-seconds 600]",
      "",
      "Creates a hash-committed, offline pre-registration artifact. It never starts collection.",
      "",
    ].join("\n"));
    return;
  }
  const repository = await realpath(resolve(dirname(fileURLToPath(import.meta.url)), "../.."));
  const tracked = git(repository, ["status", "--porcelain", "--untracked-files=no"]);
  if (tracked !== "") throw new Error("paper:campaign-plan refuses uncommitted tracked code");
  ...
  const artifact = campaignArtifact(buildKJPaperCampaign({
    ...
    gapMarketCount: nonNegativeInteger(argument("--gap-markets"), "gap-markets", 0),
  }));
  await durableWrite(outputPath, `${JSON.stringify(artifact, null, 2)}\n`);
  process.stdout.write(`${JSON.stringify({ accepted: true, campaignId: artifact.campaign.campaignId, campaignHash: artifact.campaignHash, outputPath })}\n`);
}
