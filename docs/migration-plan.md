...
## 批次 2：只读数据采集、时间归一化和耐久存储

### 要处理的模块

- BTC 5 分钟市场发现和规则快照；
- Polymarket 公共 market WS/orderbook、Chainlink RTDS、Binance 只读行情；
- snapshot/delta、PING、重连退避、connection epoch、gap/hash/resync、stale/clock health；
- 有界队列、背压、durability watermark、append-only immutable raw event；
- point-in-time normalize 和 DatasetManifest；
- Parquet/Arrow 分区及 DuckDB/Polars 只读研究接口；
...
- 成熟 Python 库：Polars、PyArrow、DuckDB、Pydantic（经单独批准后安装/锁定）。
...
- `data/raw/`、`data/processed/`、`data/fixtures/`、`data/golden/`
- `research/datasets/`
...
- 错 market/token、重复、乱序、缺失、hash 变化、gap、重连、半包/大包；
- 每 provider 的 source/send/receive/process/persist 时间和 clock skew；
- 断线、超时、无 PING/PONG、stale、主机时间跳变时 fail closed；
- 有界队列背压、慢磁盘、磁盘满、写入中断、kill -9、restart tail、hash 对拍；
- market rollover、预发现、边界秒、错误 outcome 顺序、缺规则/negRisk/fee；
- Gamma 返回重复候选、缺失当前盘、同 slug 多 condition、condition/token 冲突和多个
  同时 active 候选时必须拒绝或得到唯一可证明选择；
- point-in-time join 不能读取未来事件；同毫秒使用 ingest sequence；
- 网络层 contract 使用本地 mock/fixture，测试不得访问私有频道或订单 endpoint。

### 完成标准

- 同一 raw partition 可重放得到相同 normalized hash；
- 任何可交易 snapshot 都能证明 market identity、规则版本、四时钟、connection epoch 和
  freshness；不健康 book 不能被发布为 tradable；
- 正常关闭无未确认队列项，异常关闭可从 durability watermark 恢复并明确标记 gap；
