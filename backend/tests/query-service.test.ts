...
const result: BacktestResultV1 = {
  ...
  runId: "run-1",
  request: { schemaVersion: "backtest-request-v1", requestId: "request-1", displayName: "双波动率概率策略 · BTC 验证", description: "用于后端查询的回测谱系测试。", strategyId: "k-edge", strategyVersion: "1.0.0", datasetId: "btc", datasetVersionHash: "a".repeat(64), feeModel: "fee-v2", latencyMs: 20, initialCash: "1000", maxPosition: "100" },
  startedAtUtc: "2026-01-01T00:00:00Z",
  completedAtUtc: "2026-01-01T00:00:03Z",
  evaluationScope: { schemaVersion: "backtest-evaluation-scope-v1", split: "FINAL_TEST", horizonSeconds: 30, scenario: "BASE_1S", cohortHash: "c".repeat(64), cohortSize: 100 },
  ...
  events: [
    { eventId: "decision-1", eventTimeUtc: "2026-01-01T00:00:01Z", kind: "decision", payload: { action: "BUY", edge: "0.05", netEdge: "0.04", intendedQuantity: "5", targetPositionQuantity: "5", approvedOrderQuantity: "3", riskStatus: "REDUCED", databasePath: "/root/private.sqlite", apiKey: "never-expose" } },
...
test("query service paginates typed backtest records and strips non-public payload fields", async () => {
  ...
  assert.deepEqual(decisions.items[0]?.data, { action: "BUY", edge: "0.05", netEdge: "0.04", intendedQuantity: "5", targetPositionQuantity: "5", approvedOrderQuantity: "3", riskStatus: "REDUCED" });
...
test("run comparison is bounded, unique and contains only public summary fields", async () => {
  ...
  assert.equal(comparison[0]?.displayName, result.request.displayName);
...
test("run comparison fails closed when evaluation cohorts differ or are missing", async () => {
  const differentScope: BacktestResultV1 = { ...result, runId: "run-2", evaluationScope: { ...result.evaluationScope!, split: "VALIDATION", cohortHash: "d".repeat(64) } };
  const differing = new BackendQueryService({ async load(runId) { return runId === "run-1" ? result : differentScope; } }, { async health() { return health; }, async incidents() { return incidents; } });
  await assert.rejects(() => differing.compare(["run-1", "run-2"]), /evaluated cohort must match/u);
  const legacy: BacktestResultV1 = { ...result }; delete (legacy as { evaluationScope?: unknown }).evaluationScope;
  await assert.rejects(() => service(legacy).compare(["run-1"]), /cohort evidence is missing/u);
});
