...
export function DecisionsPage() {
  ...
  useEffect(() => {
    ...
    Promise.all([commands.getBacktestDecisions(runId, request), commands.getBacktestOrders(runId, request), commands.getBacktestFills(runId, request), commands.getBacktestSettlements(runId, request)])
      .then(([decisions, orders, fills, settlements]) => { if (active) { const next = { decisions, orders, fills, settlements }; setLedger(next); setSelected(decisions.items[0] ?? orders.items[0] ?? fills.items[0] ?? settlements.items[0] ?? null); setStatus(`已加载所选运行第 ${page} 页后端账本。`); } })
