...
export interface ParsedBinanceBookTicker {
...
function canonicalPositiveDecimal(value: unknown, field: string): string {
  const parsed = canonicalDecimalString(string(value, field));
  if (compareDecimalStrings(parsed, "0") <= 0) throw new Error(`${field} must be positive`);
  return parsed;
}

function millisecondTimestamp(value: unknown, field: string): string {
...
export function parseBinanceBookTicker(rawPayload: string): ParsedBinanceBookTicker {
  ...
  const bid = canonicalPositiveDecimal(value.b, "bookTicker.b");
  const bidSize = canonicalPositiveDecimal(value.B, "bookTicker.B");
  const ask = canonicalPositiveDecimal(value.a, "bookTicker.a");
