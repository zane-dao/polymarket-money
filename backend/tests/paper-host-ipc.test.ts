...
test("Paper host IPC is inert, closed and refuses network start without explicit approval", async () => {
  ...
  assert.deepEqual(await runtime.execute(request("get-paper-strategy-runtime")), {
    ...
    planner: { engineVersion: "kj-paper-engine-v2", journalRecordCount: 0, recoveredInputCount: 0, lastRecordHash: null, coalescedInputCount: 0, error: null, latency: {
      strategyComputation: { count: 0, p50Ms: null, p95Ms: null, maxMs: null },
      inputToDecision: { count: 0, p50Ms: null, p95Ms: null, maxMs: null },
      decisionToPaperLock: { count: 0, p50Ms: null, p95Ms: null, maxMs: null },
...
test("automatic Paper Runner derives the market and exposes only the selected strategy account", async () => {
  const root=await mkdtemp(join(tmpdir(),"paper-runner-auto-"));let discovered="";
  const runtime=new PaperHostRuntime(root,undefined,{nowMs:Date.now,feedFactory:(slug)=>{discovered=slug;return fixtureFeed(slug);}});await runtime.initialize();
  await runtime.execute(request("start-public-feed",{explicitNetworkApproval:true,runner:{strategyId:"J_FEE_AWARE",strategyVersion:"1.2.3",initialCash:"2500",maximumPosition:"125",minimumNetEdge:"0.04",strategyParameters:{edgeThreshold:0.03,maxEdge:0.2,bookParticipation:0.3}}}));
  assert.match(discovered,/^btc-updown-5m-[1-9][0-9]{9}$/u);assert.equal(Number(discovered.split("-").at(-1))%300,0);
  const status=await runtime.execute(request("get-paper-strategy-runtime")) as {canonicalAccounts:Array<{strategy:string;session:{sessionId:string;cash:string}}>};
  assert.equal(status.canonicalAccounts.length,1);assert.equal(status.canonicalAccounts[0]?.strategy,"J_FEE_AWARE");assert.equal(status.canonicalAccounts[0]?.session.cash,"2500");assert.match(status.canonicalAccounts[0]?.session.sessionId??"",/^j-fee-aware-/u);
  const journalName=(await readdir(join(root,"workbench","paper-host"))).find((name)=>name.endsWith(".jsonl"));
  assert.ok(journalName);const header=JSON.parse((await readFile(join(root,"workbench","paper-host",journalName),"utf8")).split("\n")[0]!) as {payload:{config:Record<string,string>}};
  assert.equal(header.payload.config.initialCash,"2500");assert.equal(header.payload.config.edgeThreshold,"0.03");assert.equal(header.payload.config.maxEdge,"0.2");assert.equal(header.payload.config.maximumStakeAmount,"125");assert.equal(header.payload.config.bookParticipation,"0.3");
  const firstSession=status.canonicalAccounts[0]!.session.sessionId;await runtime.execute(request("stop-public-feed"));
  assert.equal(((await runtime.execute(request("get-paper-session-status",{sessionId:firstSession}))) as {status:string}).status,"STOPPED");
  await runtime.execute(request("start-public-feed",{explicitNetworkApproval:true,runner:{strategyId:"J_FEE_AWARE",strategyVersion:"1.2.3",initialCash:"2500",maximumPosition:"125",minimumNetEdge:"0.04",strategyParameters:{edgeThreshold:0.03,maxEdge:0.2,bookParticipation:0.3}}}));
  assert.equal(((await runtime.execute(request("host-status"))) as {lifecycle:string}).lifecycle,"RUNNING");
  await runtime.close();
});
...
function settlementContext(sequence=0):KJStrategyContextV1{const decisionTime=new Date(Date.parse("2026-07-17T00:04:59.000Z")+sequence).toISOString();const ordinal=sequence*2+1;const bookStamp={schemaVersion:"receive-stamp-v1" as const,clockDomain:"host-settlement-test",localWallReceiveTime:decisionTime,localMonotonicReceiveNs:String(ordinal),localReceiveOrdinal:String(ordinal)};const signalStamp={...bookStamp,localMonotonicReceiveNs:String(ordinal+1),localReceiveOrdinal:String(ordinal+1)};const result=createKJStrategyContext({decisionTime,market:{marketId:"golden-market-1",conditionId:`0x${"a".repeat(64)}`,slug:"btc-updown-5m-1784246400",intervalStart:"2026-07-17T00:00:00.000Z",intervalEnd:"2026-07-17T00:05:00.000Z",upTokenId:"111",downTokenId:"222",active:true,closed:false,acceptingOrders:true,collectible:true,takerFeeRate:"0.07",rawPayload:"{}"},book:{state:"ACTIVE_UNVERIFIED",continuity:"UNVERIFIED",up:{bid:"0.49",ask:sequence%2===0?"0.5":"0.51",bidSize:"2",askSize:"2"},down:{bid:"0.49",ask:"0.5",bidSize:"2",askSize:"2"},receiveStamp:bookStamp},signal:{provider:"BINANCE_SPOT",price:String(67000+sequence),sourceTime:null,serverTime:null,receiveTime:decisionTime,receiveStamp:signalStamp,connectionId:"fixture-binance",inputHash:String(sequence).padStart(64,"a")}});if(!result.ready)throw new Error(result.reason);return result.context;}
...
test("host retries unresolved Gamma with a finite delay and stop cancels pending polling",async()=>{const root=await mkdtemp(join(tmpdir(),"paper-host-settlement-retry-"));const clock=new SettlementClock();const gamma=new SettlementGamma(await readFile(new URL("../../../data/fixtures/batch-06/gamma-resolved-market.json",import.meta.url),"utf8"),true);const feed=new SettlementFeed(()=>new Date(clock.now).toISOString());const runtime=new PaperHostRuntime(root,undefined,{feedFactory:()=>feed,gammaSource:gamma,nowMs:()=>clock.now,setTimer:clock.setTimer,clearTimer:clock.clearTimer,settlementRetryDelaysMs:[10]});await runtime.initialize();await runtime.execute(request("start-public-feed",{slug:"btc-updown-5m-1784246400",explicitNetworkApproval:true}));feed.observer?.strategyContext?.(settlementContext());await runtime.execute(request("get-paper-strategy-runtime"));clock.fire();await flush();assert.equal(gamma.calls,1);const waiting=await runtime.execute(request("get-paper-strategy-runtime")) as {status:string;planner:{error:string|null}};assert.equal(waiting.status,"RUNNING");assert.equal(waiting.planner.error,null,"an unresolved market is a normal settlement wait, not a runtime failure");assert.equal(clock.tasks.filter((task)=>!task.cleared).length,1);await runtime.execute(request("stop-public-feed"));assert.equal(clock.tasks.filter((task)=>!task.cleared).length,0);assert.equal(gamma.calls,1);await runtime.close();});

test("strategy persistence pressure keeps the first and latest context instead of building an unbounded stale queue",async()=>{const root=await mkdtemp(join(tmpdir(),"paper-host-latest-context-"));const clock=new SettlementClock();const feed=new SettlementFeed(()=>new Date(clock.now).toISOString());const runtime=new PaperHostRuntime(root,undefined,{feedFactory:()=>feed,nowMs:()=>clock.now});await runtime.initialize();try{await runtime.execute(request("start-public-feed",{slug:"btc-updown-5m-1784246400",explicitNetworkApproval:true}));for(let sequence=0;sequence<100;sequence+=1)feed.observer?.strategyContext?.(settlementContext(sequence));const status=await runtime.execute(request("get-paper-strategy-runtime")) as {planner:{journalRecordCount:number;coalescedInputCount:number}};assert.ok(status.planner.coalescedInputCount>=98);assert.ok(status.planner.journalRecordCount<=4);const journalName=(await readdir(join(root,"workbench","paper-host"))).find((name)=>name.endsWith(".jsonl"));assert.ok(journalName);const records=(await readFile(join(root,"workbench","paper-host",journalName),"utf8")).trim().split("\n").map((line)=>JSON.parse(line) as {payloadType:string;payload:{decisionTime?:string}});assert.equal(records.at(-1)?.payload.decisionTime,settlementContext(99).decisionTime);}finally{await runtime.close();}});
