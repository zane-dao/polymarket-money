...
function points(values: readonly number[]): string {
  ...
  return values.map((value, index) => `${(index / (values.length - 1)) * 100},${92 - ((value - min) / spread) * 78}`).join(" ");
}
...
export function LineChart({ series, height = 260 }: { series: readonly Series[]; height?: number }) {
  return <div className="chart" style={{ height }}>
    <svg viewBox="0 0 100 100" preserveAspectRatio="none" role="img" aria-label={series.map((item) => item.label).join("、")}>
      ...
      {series.map((item) => <polyline key={item.label} points={points(item.values)} fill="none" stroke={item.color} strokeWidth="1.3" vectorEffect="non-scaling-stroke" />)}
    </svg>
