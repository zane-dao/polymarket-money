...
- 用户同时提出多个彼此独立、文件边界清晰的问题时，应使用子代理并行实现或核查；主代理负责统一
  设计、冲突处理、验证与最终交付。不得因为当前主线要求逐项展示而停止其他已授权的独立工作。
- 涉及同一组件、同一合同或强依赖顺序的改动仍应串行，避免多个代理同时修改同一事实来源。
...
- 允许为当前任务在项目根目录新建文件或文件夹，但执行前必须说明新建原因、用途和影响范围；
  能放入现有职责目录时不得为了方便而新增根级入口。用户明确拒绝后不得新建。
