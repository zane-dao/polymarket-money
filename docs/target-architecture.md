...
| 当前文件/目录 | 已具备 | 仍缺失或不可信 |
|---|---|---|
| `execution/src/domain/index.ts` | 已有 Market、OutcomeToken、OrderBook、Trade、Order、Fill、Position、Balance、SignalDecision、RiskDecision、Settlement 和四个规范时间名 | 金额/价格/份额仍是无单位 `number`；缺 market epoch/rules/source/tick/negRisk/fee、book health/hash/connection epoch、fill role/finality；`exchangeTimestamp` 被强制必填而无源时间应可空，`persistTimestamp` 反而可选 |
...
| `execution/src/risk/index.ts` | 已列单笔、单市场、日亏、滑点、挂单、stale、WS 和幂等配置 | 数值是 placeholder，使用无单位 number；没有 RiskEngine 实现、配置 schema/invariant、cash reservation、VaR/CVaR 或 fail-closed 组合测试 |
| `execution/src/storage/`、`monitoring/` | 空目录明确避免过早接线 | 尚无 event journal、ledger、dataset manifest、恢复或健康状态机 |
...
当前 Git 仓库已初始化但尚无首次提交，文件仍是 untracked 状态。第一批只添加 clean-room
离线模型和测试，不把它误报成持久账本、数据采集、回测或实盘能力。
...
| 字段 | 含义 |
|---|---|
| `exchangeTimestamp` | 交易所或数据源声明的事件时间；源没有提供时必须显式为空 |
| `receiveTimestamp` | 本进程收到完整消息的单调映射 UTC 时间 |
| `processTimestamp` | 完成解析并进入领域处理的时间 |
| `persistTimestamp` | 事件被耐久写入后的时间 |

禁止用一个 `ts` 混合四种语义。策略的“当前时间”必须作为输入传入；过期保护以
`receiveTimestamp` 与显式决策时间计算，并同时监控 exchange/receive clock skew。

不同 provider 的额外时刻不能硬塞进一个字段：例如 Chainlink RTDS 的
`payload.timestamp` 映射为价格源的 `exchangeTimestamp`，顶层消息发送时间保留为可选
`providerServerTimestamp`；本地接收仍是 `receiveTimestamp`。派生的 SignalDecision
应另带 `decisionTimestamp`（其事件 envelope 的 `processTimestamp`）和
`inputWatermarkReceiveTimestamp`，证明决定只看到了哪个接收水位。这样既保持四个规范
生命周期时间名，又保留 source/server/receive/decision 的因果区别。
...
2. **只读数据采集、时间和存储**：再实现无凭据的市场发现、公共订单簿、Chainlink/
   Binance、不可变 raw event、point-in-time normalization 和数据集 provenance。
