...
function rechain(records:readonly EvidenceRecord[]):EvidenceRecord[]{const output:EvidenceRecord[]=[];for(const record of records){const previousHash=output.at(-1)?.recordHash??GENESIS;const core={ordinal:record.ordinal,kind:record.kind,observedAtUtc:record.observedAtUtc,marketId:record.marketId,detail:record.detail,latencyMs:record.latencyMs,ageMs:record.ageMs,previousHash};output.push(Object.freeze({...core,recordHash:digest(core)}));}return output;}
