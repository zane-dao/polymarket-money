...
test("operator can run automatic Paper and inspect comparable arena results", async ({ page }, testInfo) => {
  ...
  await page.getByRole("navigation",{name:"主导航"}).getByRole("button",{name:/策略竞技场/}).click();
  await page.getByRole("button",{name:"比较所选运行（2）"}).click();
  await expect(page.getByText("比较完成：2 个运行使用相同数据分组、时间跨度、执行场景和样本集合")).toBeVisible();
  await expect(page.getByRole("cell",{name:"18.4"})).toBeVisible();
  await expect(page.getByText("仅作描述",{exact:true}).first()).toBeVisible();
  if (testInfo.project.name === "desktop-chromium") {
