...
├── src-tauri/             暂停使用的桌面壳代码；不是当前入口或主链
...
- `frontend/src/`：已有 React 应用壳、8 个独立页面模块、共享 UI/图表组件、框架无关状态机、
  ...
  不直接访问数据库、文件或公网。
- `src-tauri/src/`：保留供未来单独恢复，当前 Web 产品不经过它，亦不以它作为验收前提。
- `backend/`：已有 `market-data/`、`backtest/`、`risk/`、`storage/` 和测试；原顶层
  `execution/` 的 TypeScript 实现已整体归入 `backend/core/`，不再保留顶层 execution 目录。
- `strategies/catalog.json`：策略身份、元数据、参数 Schema、允许模式和执行器键的唯一登记源；
  TypeScript catalog 与 Python 执行视图从它加载，不再手写两份清单。`strategies/src/` 拥有统一
  策略输入/目标仓位输出合同与 K/J context/warmup；J/K/L 的确定性研究实现位于
...
```text
frontend -> fixed loopback Web commands -> automatic Paper Runner
                                           |
public feed -> normalized input -> strategy -> risk -> PaperSimulationEngine
                                           |              |
                                           + journal/outbox/ledger/official settlement
```

- `frontend` 只展示状态和提交显式请求，不读取数据库、文件或网络。
- 当前入口是固定 localhost Web API；Tauri 不在当前调用链。
...
- 目标架构要求回测、Paper 与未来执行只依赖同一策略公共接口。当前 J/K 尚未完全达到：离线回测
  仍走 Python `run_kj_paper`，Paper 走 TypeScript `KJPaperEngine`；两者只有 catalog、参数和 golden
  一致性是共享的。下一次策略管线改造必须删除其中一份决策实现，而不是再增加适配层。
...
- 实时 Paper 由 Web 进程内唯一 `PaperHostRuntime` 持有。用户只选择策略版本、初始资金、最大仓位、
  最低净优势并启动/停止；slug、host、session、journal、outbox、手工订单与手工结算不属于主流程。
