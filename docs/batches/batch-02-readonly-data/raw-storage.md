...
Binance manifest 还把“provider 传输范围”和“有效业务过滤”分开记录：默认
`transportScope=btc-only`；有限后备验证可为 `all-symbols-quarantine`，但两者的
`symbolFilter` 都必须是 `btcusdt`。Python verifier 与 TypeScript writer 会交叉核对实际
subscription 和这两个字段，防止清单把全符号订阅伪装成源端 BTC 过滤。
