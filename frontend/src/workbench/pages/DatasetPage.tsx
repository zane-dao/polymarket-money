...
export function DatasetPage() {
  ...
  return <><PageHeader title="数据集管理（Datasets）" subtitle="先选择可信的数据版本；添加和归一化操作收在次级区域。" action={<button className="button button--primary" disabled={commands === null || busy} onClick={() => void scan()}>重新扫描</button>} />
    ...
    {detail === null ? null : <Panel title={`${detail.displayName} · 详情`} english="Backend-Verified Detail" action={<Badge tone="good">可用于选择</Badge>}><div className="metadata-grid"><span>数据覆盖<b>{formatUtc(detail.startTimeUtc)}<br />{formatUtc(detail.endTimeUtc)}</b></span><span>记录 / 隔离<b>{formatCount(detail.rowCount)} / {formatCount(detail.quarantineCount)}</b></span><span>生成时间<b>{formatUtc(detail.publishedAtUtc)}</b></span></div><details className="inline-details"><summary>查看技术身份</summary><dl className="compact-dl"><dt>数据集 ID</dt><dd>{detail.datasetId} <CopyButton value={detail.datasetId}/></dd><dt>完整版本哈希</dt><dd className="break-all">{detail.versionHash} <CopyButton value={detail.versionHash}/></dd><dt>原始数据策略</dt><dd>{detail.rawDataPolicy}</dd></dl></details></Panel>}
    <details className="technical-details"><summary>添加或归一化历史数据</summary><div className="technical-details__body stack"><Panel title="归一化原始历史数据"><div className="adaptive-form"><label className="field-span-4">数据集 ID<input aria-label="原始数据集 ID" value={rawDatasetId} placeholder="btc-five-minute-local" onChange={(event) => setRawDatasetId(event.target.value)} /></label><label className="field-span-8">CSV / JSON / NDJSON 文件或平铺目录<input aria-label="原始历史数据路径" value={rawInputPath} placeholder="/absolute/path/to/raw-events.ndjson" onChange={(event) => setRawInputPath(event.target.value)} /></label><div className="field-span-12 form-actions"><button className="button button--primary" aria-label="Web 后端操作" disabled={commands === null || busy || rawInputPath.trim() === "" || rawDatasetId.trim() === ""} onClick={() => void normalizeRaw()}>归一化并发布</button></div></div><p>后端校验统一事件 schema；原始输入保持只读。</p></Panel><Panel title="登记已有的标准化发布"><div className="adaptive-form"><label className="field-span-9">外部 normalized 发布根<input aria-label="外部 normalized 发布根" value={sourceDirectory} placeholder="/absolute/path/to/normalized" onChange={(event) => setSourceDirectory(event.target.value)} /></label><div className="field-span-3 form-actions"><button className="button button--primary" aria-label="后端操作" disabled={commands === null || busy || sourceDirectory.trim() === ""} onClick={() => void registerSource()}>由后端添加</button></div></div></Panel></div></details>
  </>;
}
